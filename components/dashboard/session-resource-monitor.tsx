...
function getStableMetric(seed: string, min: number, range: number) {
      ...
      <CardContent className="space-y-6">
        {activeSessions.length === 0 ? (
          ...
          activeSessions.map(session => {
            const cpuUsage = getStableMetric(`${session.id}-cpu`, 10, 30)
            const ramUsage = getStableMetric(`${session.id}-ram`, 20, 40)
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs text-gray-500">
                      <span>CPU Usage</span>
                      <span className="font-medium text-gray-700">{cpuUsage.toFixed(1)}%</span>
                    </div>
                    <Progress value={cpuUsage} className="h-1.5" />

                    <div className="flex justify-between pt-1 text-xs text-gray-500">
                      <span>RAM Usage</span>
                      <span className="font-medium text-gray-700">{ramUsage.toFixed(1)}%</span>
                    </div>
                    <Progress value={ramUsage} className="h-1.5" variant="warning" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Progress
                      value={((session.scratch?.used || 0) / (session.scratch?.quota || 100)) * 100}
                      className="h-1.5"
                    />
                  </div>
                </div>
